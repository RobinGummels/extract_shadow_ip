...
class extract_shadow_ip(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config: Optional[dict[str, Any]] = None):
        ...
        # Add output layer
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.output_ip, "Output layer")
        )
    ...
    def processAlgorithm(
        ...
        # Retrieve the inputs
        input_buildings = self.parameterAsSource(parameters, self.input_buildings, context)
        ...
        if sink is None:
            raise QgsProcessingException(self.invalidSinkError(parameters, self.output_ip))

        # Compute the number of steps to display within the progress bar and
        # get features from source
        total = 100.0 / input_buildings.featureCount() if input_buildings.featureCount() else 0
        ...
        yes = [1000, 1010, 1020, 1021, 1022, 1023, 1024, 1025, 1100, 1110, 1120, 1121, 1122, 1123, 1130, 1131, 1210, 1220, 1221, 1222, 1223, 1310, 1311, 1312, 2020, 2030, 2040, 2070, 2071, 2072, 2073, 2074, 2080, 2081, 2082, 2083, 2090, 2130, 2180, 2310, 2320, 2411, 2421, 2732, 3000, 3010, 3011, 3012, 3013, 3014, 3015, 3016, 3017, 3018, 3019, 3020, 3021, 3022, 3023, 3024, 3030, 3031, 3034, 3037, 3040, 3041, 3042, 3043, 3044, 3045, 3046, 3047, 3048, 3050, 3051, 3052, 3053, 3054, 3060, 3061, 3062, 3063, 3064, 3065, 3066, 3071, 3072, 3073, 3075, 3100, 3200, 3240, 3241, 3242]
        no = [1313, 2060, 2092, 2093, 2094, 2095, 2140, 2141, 2142, 2143, 2210, 2211, 2212, 2213, 2220, 2442, 2444, 2460, 2461, 2462, 2463, 2464, 2465, 2612, 2621, 2721, 2723, 2724, 2726, 2727, 2728, 3074, 3262, 3264]
        maybe = [2000, 2010, 2050, 2051, 2052, 2053, 2054, 2055, 2056, 2091, 2100, 2110, 2111, 2112, 2113, 2114, 2120, 2121, 2131, 2150, 2160, 2170, 2171, 2172, 2200, 2400, 2410, 2412, 2420, 2422, 2423, 2424, 2430, 2431, 2440, 2441, 2443, 2450, 2451, 2500, 2501, 2510, 2511, 2512, 2513, 2520, 2521, 2522, 2523, 2527, 2528, 2529, 2540, 2560, 2570, 2571, 2580, 2590, 2591, 2600, 2610, 2611, 2620, 2622, 2623, 2700, 2720, 2729, 2735, 2740, 2741, 2742, 3032, 3033, 3035, 3036, 3038, 3070, 3080, 3081, 3082, 3090, 3091, 3092, 3094, 3095, 3097, 3098, 3210, 3211, 3212, 3220, 3221, 3222, 3230, 3260, 3261, 3263, 3270, 3271, 3272, 3273, 3280, 3281, 3290, 9998]
        ...
        for current, building_feature in enumerate(building_features):
            gfkzshh = str(building_feature.attribute("gfkzshh")) if building_feature.attribute("gfkzshh") is not None else ""
            importance = 2 # Fallback if gfkzshh is missing
            for code in yes:
        ...
        for current, building_feature in enumerate(building_features):
            ...
            if closest_point:
                ...
                # Attribute + importance aus Liste
                importance = importance_list[current]
